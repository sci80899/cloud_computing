...
from .models import Movie,User,Review,Pal
...
def movie_detail(request,movie_id):
    ...
    pal= Pal.objects.filter(movie_id=movie_id)
    print(review)
    ...
    return render(request,'movie_detail.html',{'article_list': article_list, 'data': data,'movie':movie,'review':review,'pal':pal})
...
def movie_list(request):
    ...
    return render(request, 'movie_list.html',{'article_list': article_list, 'data': data})
...
def loginCheck(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(username=username,password=password).exists():
            return render(request,'movie_list.html', {'username':username})
def login(request):
    return render(request,'login.html')
...
def search(request):
   ...
   #search_movies=Movie.objects.filter(title__icontains=search_word)
   os.system("python3 test.py '%s'"%search_word)
   search_movies = Movie.objects.filter(title__icontains=search_word)
   context={}
